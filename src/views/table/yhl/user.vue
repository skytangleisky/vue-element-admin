  ...
  >
    ...
    >
      ...
      >
        ...
        <el-form-item
          ...
          prop="user_name"
        >
          <el-input v-model="addData.user_name" />
        </el-form-item>
        <el-form-item
          label="密码"
          prop="password"
        ...
        <el-form-item
          ...
          prop="real_name"
        >
          <el-input v-model="addData.real_name" />
        </el-form-item>
        <el-form-item
          label="手机号"
          prop="phone"
        ...
        <el-form-item label="是否启用">
          <el-switch v-model="addData.is_active" />
        </el-form-item>
      </el-form>
      <div class="button">
    ...
    >
      ...
      >
        ...
        <el-form-item
          ...
          prop="real_name"
        >
          <el-input v-model="editData.real_name" />
        </el-form-item>
        <el-form-item
          label="手机号"
          prop="phone"
        ...
        <el-form-item
          ...
          prop="company"
        >
          <el-input v-model="editData.company" />
        </el-form-item>
        <el-form-item
          label="统一社会信用代码"
          prop="scc"
        >
          <el-input v-model="editData.scc" />
        </el-form-item>
        <el-form-item label="营业执照">
          <!--
              on-preview: 点击文件列表中已上传的文件时的钩子
        ...
        <el-form-item label="是否启用">
          <el-switch v-model="editData.is_active" />
        </el-form-item>
      </el-form>
      <div class="button">
    ...
    >
      ...
      <el-descriptions :column="1">
        <el-descriptions-item label="账号">{{ detailData.user_name }}</el-descriptions-item>
        ...
        <el-descriptions-item label="用户类型">{{ detailData.user_type }}</el-descriptions-item>
        <el-descriptions-item label="公司名称">{{ detailData.company }}</el-descriptions-item>
        <el-descriptions-item label="统一社会信用代码">{{ detailData.scc }}</el-descriptions-item>
...
import { insertUser, del, deleteUser, updateUser, selectUser } from '/src/api/table/t_user'
...
export default {
  ...
  data() {
    ...
    var phoneValidator = async(rule, value, callback) => {
      const existCount = await this.addValIsExist(rule.field, value)
      if (existCount > 0) {
        callback(new Error('该手机号已存在'))
      }
    }
    var emailValidator = async(rule, value, callback) => {
      const existCount = await this.addValIsExist(rule.field, value)
      if (existCount > 0) {
  ...
  methods: {
    ...
    searchSuperUser(val) {
      this.searchValChange(val, 2, 'is_active')
    },
    // 搜索下拉框值改变时即搜索条件改变时触发
    ...
    insertList(data) {
      ...
      this.$refs.addForm.validate(async valid => {
        if (valid) {
          ...
          const res = await insertUser(this.formatDataBase(data))
          if (this.resResult(res, '数据')) {
            /* 将用户uuid添加进文件表中*/
            const newData = await this.getAddData(res, selectUser)
    ...
    detailPageClose() {
      ...
      // 转变is_active的值
      switch (this.detailData.is_active) {
        case true:
          this.detailData.is_active = '是'
          break
        case false:
          this.detailData.is_active = '否'
          break
        default:
          break
      }
    ...
    async getDetailData(data) {
      ...
      switch (this.detailData.is_active) {
        case '是':
          ...
          break
        case '否':
          this.detailData.is_active = false
          break
        default:
          break
      }
      this.detailData.city = this.detailData.city.join(' ')
    ...
    cancelEdit() {
      ...
      // 转变is_active的值
      switch (this.editData.is_active) {
        case true:
          this.editData.is_active = '是'
          break
        case false:
          this.editData.is_active = '否'
          break
        default:
          break
      }
      const tmp = []
      this.editData.city.forEach(item => {
        tmp.push(CodeToText[item])
      })
      this.editData.city = tmp.join(',')
      this.editData.city = this.editData.city.split(',')
    ...
    async getEditData(data) {
      ...
      switch (this.editData.is_active) {
        case '是':
          this.editData.is_active = true
          break
        case '否':
          this.editData.is_active = false
          break
        default:
          break
      }
    ...
    updateList() {
      ...
      this.$refs.editForm.validate(async valid => {
        if (valid) {
          ...
          // console.log("this.editData", this.editData.city)
          const res = await updateUser(this.formatDataBase(this.editData))
    ...
    getCityCode(data) {
      const cityCode = []
      const provinceCode = TextToCode[data.city[0]].code
      const countryCode = TextToCode[data.city[0]][data.city[1]].code
      cityCode.push(provinceCode, countryCode)
      data.city = cityCode
    },
    // 删除
    deleteList(data) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        const res = await deleteUser(deleteDate)
        console.log(res)
        // 响应结束后的反馈信息
    ...
    formatList(oldList) {
      ...
      oldList.forEach(item => {
        ...
        if (item.last_login) {
          item.last_login = item.last_login.slice(0, 19)
        }
        newList.push(item)
      })
      return newList
