     ...
     -->
      ...
      <div class="search">
        ...
        >
          ...
          <el-form-item label="是否视距内飞行:">
            <el-select
              v-model="searchForm.VLOS"
              @change="searchVLOS"
            >
              <el-option
                label="全部"
...
import { insertPlan, delPlan, updatePlan, selectPlan } from '/src/api/table/t_plan_application.js'
// 过滤表单模块
...
/** 公共方法
 * searchByCondition：条件搜索
 * fuzzySearch：模糊搜索
 * getPageParam：获取分页参数
 * tableSort：表格字段排序
 * getTableData：封装查询接口
 */
import { searchByCondition, fuzzySearch, getPageParam, tableSort, getTableData } from './components/commonFun.js'
...
export default {
  ...
  components: {
    ...
    tableModel,
    Pagination
    // "airspaceMap": () => vueRemoteImport('/lcc/entry.html', 'test')
  },
  data() {
    return {
      // 模糊搜索匹配的字段
  ...
  methods: {
    ...
    // 查询数据
    async selectList(listQuery) {
      const selResults = await getTableData(listQuery, selectPlan)
      // 格式化数据
      this.list = this.formatList(selResults.tableData)
      this.total = selResults.total
    },
    // 模糊搜索
    searchList(condition) {
      this.listQuery = fuzzySearch(condition, this.listQuery, this.searchContent)
      this.selectList(this.listQuery)
    },
    // 条件搜索
    searchAppType(val) {
      this.listQuery = searchByCondition(val, 3, 'app_type', this.listQuery)
      this.selectList(this.listQuery)
    },
    searchEmergency(val) {
      this.listQuery = searchByCondition(val, 2, 'emergency', this.listQuery)
      this.selectList(this.listQuery)
    },
    searchVLOS(val) {
      this.listQuery = searchByCondition(val, 2, 'VLOS', this.listQuery)
      this.selectList(this.listQuery)
    },
    // 分页
    pagination(vals) {
      this.listQuery = getPageParam(vals, this.listQuery)
      this.selectList(this.listQuery)
    },
    // 排序
    tableSort(vals) {
      this.listQuery = tableSort(vals, this.listQuery)
      this.selectList(this.listQuery)
    },
    ...
    insertList(data) {
      ...
      this.$refs.addForm.validate(async valid => {
        if (valid) {
          ...
          const res = await insertPlan(data)
          console.log(res)
    ...
    updateList() {
      ...
      this.$refs.editForm.validate(async valid => {
        if (valid) {
          ...
          const res = await updatePlan(this.formatDataBase(this.editData))
          if (this.resResult(res, '数据')) {
            this.selectList(this.listQuery)
          }
          this.editPageClose()
    ...
    deleteList(data) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        */
        ...
        const res = await delPlan(deleteDate)
    ...
    formatList(oldList) {
      ...
      oldList.forEach(item => {
        ...
        item.end_time = item.end_time.slice(0, 19)
        newList.push(item)
      })
      return newList
    },
