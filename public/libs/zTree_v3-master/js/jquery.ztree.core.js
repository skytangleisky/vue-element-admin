    ...
    view = {
      ...
      asyncNode: function (setting, node, isSilent, callback) {
        ...
        if (node) {
          ...
          if(node.iconSkinLoading){// tanglei
              icoObj.attr({"style": "", "class": node.iconSkinLoading})
          }else icoObj.attr({"style": "", "class": consts.className.BUTTON + " " + consts.className.ICO_LOADING});
        }
      ...
      /*makeNodeIcoClass: function (setting, node) {// tanglei
        var icoCss = ["ico"];
        if (!node.isAjaxing) {
          var isParent = data.nodeIsParent(setting, node);
      ...
      makeNodeIcoClass: function (setting, node) {
        ...
        if (!node.isAjaxing) {
            ...
            if (isParent) {
                icoCss.push(node.open ? consts.folder.OPEN : consts.folder.CLOSE);
                if(node.open && node.iconSkinOpen) {
                    return node.iconSkinOpen;
                }else if(node.iconSkinClose){
                    return node.iconSkinClose;
                }
            } else {
                icoCss.push(consts.folder.DOCU);
                if(node.iconSkinDocu){
                    return node.iconSkinDocu;
                }
            }
        }
        return consts.className.BUTTON + " " + icoCss.join('_');
      ...
      // replaceIcoClass: function (node, obj, newName) { // tanglei
      ...
      replaceIcoClass: function (node, obj, newName) {
        ...
        if(newName==consts.folder.OPEN){
          node.iconSkinOpen && obj.attr("class",node.iconSkinOpen);
        }else if(newName==consts.folder.CLOSE) {
          node.iconSkinClose && obj.attr("class", node.iconSkinClose);
        }
      },
    ...
    $.fn.zTree={
      ...
      init: function (obj, zSetting, zNodes) {
        ...
        try{
          ...
          setting.treeId = zt.tanglei++;//tanglei
        }
        setting.treeObj = $(obj);
        setting.treeObj.empty();
        ...
        var zTreeTools = {
          ...
          removeNode: function (node, callbackFlag) {
            ...
            // if (callbackFlag && tools.apply(setting.callback.beforeRemove, [setting.treeId, node], true) == false) return;
            if(callbackFlag){
              setting.callback.beforeRemove(setting.treeId, node).then((res)=>{
                if(res){
                  view.removeNode(setting, node);
                  if (callbackFlag) {
                    this.setting.treeObj.trigger(consts.event.REMOVE, [setting.treeId, node]);
